%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%Algoritmo para fazer a sintonização do controlador PID utilizando ARPSO
%%Mario Andrés Pastrana Triana
%%Estudante de mestrado da UnB
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [bestfitness, ys]=ARPSO (x_min,x_max,N,S,maxiter,sp)
...
%% Plotanto as partÃ­culas a cada iteraÃ§Ã£o
...
%% Atualização da posição e velocidade das particulas
    ...
    for j = 1:N   % For das dimensões         
        for i=1:S % For das particulas
            ...
            v(i,j) = w*v(i,j) + sig*c1*r1*(y(i,j)-x(i,j)) + sig*c2*r2*(ys(j) - x(i,j)); % Equação da velocidade do PSO
